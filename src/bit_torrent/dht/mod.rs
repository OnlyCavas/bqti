...
use crate::bit_torrent::torrent::metainfo::InfoHash;
...
pub trait TorrentDht {
    ...
    async fn get_peers(&self, info_hash: &Key) -> Result<Vec<SocketAddr>, KademliaError>;
}
...
impl TorrentDht for Kademlia {
    async fn announce(&self, info_hash: Key) -> Result<(), KademliaError> {
        ...
        self.store(info_hash, KademliaData::Peers(HashSet::from([host_addr])))
            .await
    }
